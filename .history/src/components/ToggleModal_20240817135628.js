  ...
  return (
    ...
    <div className='flex justify-center items-center'>
      <div name='button-group' className='flex rounded-md bg-slate-200'>
        <div name='button-right' onClick={handleToggle} className={`w-[100px] rounded-md ${selected ? 'red' : 'gray'}`}>살래요</div>
        <div name='button-left' onClick={handleToggle} className={`w-[100px] rounded-md ${!selected ? 'red' : 'gray'}`}>팔래요</div>
      </div>
    </div>
  );
};
