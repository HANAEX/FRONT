...
const ChartCard = ({ index, name, increase, imageUrl, currentPrice, date, chartData }) => {
  const [heart, setHeart] = useState(false);
  const handleHeart = () => {
    setHeart(!heart)
  }
  return (
    <div name="card-container" className='w-[250px] h-[350px] px-8 py-4 bg-slate-50 rounded-2xl'>
      ...
      <ResponsiveContainer width="100%" height="45%">
        ...
        <AreaChart width={300} height={100} data={chartData}>
          <defs>
            <linearGradient id={`colorPv-${increase >= 0 ? 'positive' : 'negative'}`} x1="0" y1="0" x2="0" y2="1">
              <stop offset="5%" stopColor={increase >= 0 ? '#FF0000' : '#524CFF'} stopOpacity={0.8}/>
              <stop offset="95%" stopColor={increase >= 0 ? '#FF0000' : '#524CFF'} stopOpacity={0}/>
            </linearGradient>
          </defs>
          <Tooltip />
          <Area type="monotone" dataKey="uv" stroke={increase >= 0 ? '#FF0000' : '#524CFF'} fillOpacity={2} fill={`url(#colorPv-${increase >= 0 ? 'positive' : 'negative'})`}/>
        </AreaChart>
      </ResponsiveContainer>
      ...
      <Link to={"/MainPageDetail/" + {data.name} } className='text-sm font-bold flex justify-end text-slate-500 py-2'>자세한 분석 보러가기</Link>
    </div>
  )
}
