  ...
  return (
    ...
    <div className='flex justify-center items-center'>
      <div name="button-group" className='flex rounded-md bg-slate-200'>
        <button onClick={handleToggle} className={`w-100px rounded-md ${selected ? 'red' : 'gray'}`}>살래요</button>
        <button onClick={handleToggle} className={`w-100px rounded-md ${!selected ? 'red' : 'gray'}`}>팔래요</button>
      </div>
    </div>
