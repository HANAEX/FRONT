...
const NewsCard = ({ state, url, title, content, image_url, result }) => {
  return (
    <Link to={"/NewsPageDetail/" + "'" +  url + "'"} name="card-container" className='w-[300px] h-[310px] p-3 bg-slate-50 rounded-2xl'>
      <img src= { encodeURIComponent(image_url) } alt='News Image'  className='object-cover rounded-2xl w-full h-[140px]'/>
