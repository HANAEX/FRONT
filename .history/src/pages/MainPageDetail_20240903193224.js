...
const MainPageDetail = () => {
    ...
    const getUseHistory = async () => {
      try {
        ...
        setCurrentInvestPrice(
          parseFloat(result.data.todayData[0].cash_buy.toFixed(2))
        );
  ...
  useEffect(() => {
    ...
    if (
      ...
      roundedVendWonValue === roundedCurrentSellPrice
    ) {
      setShowClander(false);
    } else {
      setShowClander(true);
    }
  }, [wonValue, vendWonValue, currentInvestPrice, currentSellPrice]);
