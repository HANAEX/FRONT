...
const SemanticPage = () => {
  ...
  useEffect(() => {
    const getUseHistory = async () => {
      try {
        ...
        const result = await axios.get("https://a8068697-21bf-44f7-bf02-fccbc7115c44.mock.pstmn.io//hana/semantic");
  ...
  const filteredData = consumData.filter(data => {
    return selected ? data.semantic === "긍정적" : data.semantic === "부정적";
  });
