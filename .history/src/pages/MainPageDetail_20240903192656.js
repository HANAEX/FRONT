...
const MainPageDetail = () => {
  ...
  useEffect(() => {
    const getUseHistory = async () => {
      try {
        ...
        setCurrentInvestPrice(parseFloat(result.data.todayData[0].cash_buy.toFixed(2)));
        setCurrentSellPrice(result.data.todayData[0].cash_sell);
        setWonValue(parseFloat(result.data.todayData[0].cash_buy.toFixed(2)));
  ...
  // calender 로직
  useEffect(() => {
    console.log("wonValue:", wonValue, "currentInvestPrice:", currentInvestPrice);
  ...
  useEffect(() => {
    console.log("wonValue:", vendWonValue, "currentInvestPrice:", currentSellPrice);
    if (vendWonValue === currentSellPrice) {
      setShowClander(false);
    } else {
      setShowClander(true);
    }
  ...
  return (
    <div className="w-[960px] bg-slate-500 flex flex-col py-1 px-10">
      ...
      <div className="w-full px-48 py-2 rounded-lg my-2 bg-slate-100">
        <div className="flex justify-center my-3">
          <MainDropDown changeState={changeState} setChangeState={setChangeState}/>
        </div>
        <BuySell
