...
const NewsPageDetail = () => {
  ...
  return (
    <div className="w-[1140px] bg-slate-500 flex flex-col py-1 px-10 mt-10">
      ...
      {/* 요약box */}
      <div name="gpt-box" className='bg-slate-200 p-6 rounded-md'>
        {gpttext === true ? (
          <Text>
            {truncateText(gpt_text, 100)}
          </Text>
        ) : (
          <Text>
            {gpt_text}
        ...
        <div className='flex justify-center mt-2'>
          <Text name="show-gpttext" className='text-base text-slate-700' onClick={handleGpttext}>
            자세히보기 <TiArrowSortedDown />
          </Text>
        </div>
      </div>
