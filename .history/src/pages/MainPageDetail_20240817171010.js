...
const MainPageDetail = () => {
  ...
  useEffect(() => {
    const getUseHistory = async () => {
      try {
        const result = await axios.get("https://a8068697-21bf-44f7-bf02-fccbc7115c44.mock.pstmn.io//hana/mainpagedetail");
        setConsumData(result.data);
        setCurrentInvestPrice(consumData.invest);
        setCurrentSellPrice(consumData.sell);
  ...
  return (
    <div className="w-[960px] bg-slate-500 flex flex-col py-1 px-10">
      ...
      <div className="w-full flex justify-center px-48 py-2 rounded-lg my-2 bg-slate-100">
        ...
        <Stat className="flex justify-center">
          <StatLabel>{currentSellPrice}</StatLabel>
