...
const NewsPageDetail = () => {
  ...
  return (
    <div className="w-[1140px] bg-slate-500 flex flex-col py-1 px-10 mt-10">
      ...
      <div name="content-box" className='h-2 bg-slate-100 p-3'>
        <Text >
          이날 원·달러 환율이 상승한 배경에는 스위스 중앙은행의 2회 연속 금리 인하, 영국 영란은행(BOE)의 완화적 금리 동결 등 미국과의 통화정책 차별화가 있다.앞서 스위스중앙은행은 이달 20일(현지시간 기준) 정례 통화정책회의를 통해 기준금리를 1.25%로 25bp 인하한다고 발표했다. 올 3월에 이어 두 번째로 금리를 내린 것이다. 또한 스위스에 이어 영국도 8월에 금리를 내릴 것이라는 기대감이 나오고 있다.
          중국 위안화와 일본 엔화도 원화 약세에 영향을 미치고 있다. 최근 중국은 주요 경제 지표들이 부진해 앞으로 경기 부양을 위해 금리를 내릴 수 있다는 전망이 나온다. 이에 위안화 약세 압력이 커진 상황이다. 일본 중앙은행도 이달 열린 통화정책회의에서 기준금리를 0~0.1%로 동결한 후 장기국채 매입 감액에 대한 계획이나 추가 금리 인상에 대한 제스처가 없다.
        </Text>
      </div>
    </div>
  );
