...
const MainPageDetail = () => {
    ...
    const getUseHistory = async () => {
      try {
        ...
        setCurrentInvestPrice(
          result.data.todayData[0].cash_buy
        );
  ...
  useEffect(() => {
    const result = parseFloat(exchangeValue) * parseFloat(vendWonValue);
    setCalculatedValue(isNaN(result) ? 0 : result.toFixed(2)); // 소수점 2자리까지 결과 표시
  }, [exchangeValue, vendWonValue]);
  ...
  useEffect(() => {
    ...
    if (
      roundedWonValue === roundedCurrentInvestPrice
    ) {
  ...
  const handlePurchaseClick = () => {
    ...
    // }
    const roundedWonValue = parseFloat(parseFloat(wonValue).toFixed(2));
    const roundedCurrentInvestPrice = parseFloat(parseFloat(currentInvestPrice).toFixed(2));
    const roundedVendWonValue = parseFloat(parseFloat(vendWonValue).toFixed(2));
    const roundedCurrentSellPrice = parseFloat(parseFloat(currentSellPrice).toFixed(2));

    if (roundedWonValue === roundedCurrentInvestPrice || roundedVendWonValue === roundedCurrentSellPrice) {
        setModalType("DirectModal");
    } else {
        setModalType("ReserveModal");
    }
    ...
    <div className="w-[960px] bg-slate-500 flex flex-col py-1 px-10">
      ...
      <div className="w-full px-48 py-2 rounded-lg my-2 bg-white">
        <div className="flex justify-center items-center my-3">
          ...
          />
          <Text className='text px-3 border-2 text-slate-600 border-green-500 rounded-full'> Lv3 우대 80% </Text>
        </div>
      ...
      <div className="flex flex-col items-center bg-white rounded-lg py-5">
        ...
        <div className="flex gap-3 py-1">
          <div className="flex items-center gap-2">
            <div className="w-4 h-4 rounded-full" style={{ backgroundColor :  "#b6e9e5" }}></div>
            <Text>최저가-최고가</Text>
          </div>
          <div className="flex items-center gap-2">
            <div className="w-4 h-4 border-2 rounded-full" style={{ backgroundColor :  "#b6e9e5", borderColor : "#32B3B7" }}></div>
            <Text>매매기준율(종가)</Text>
          </div>
        </div>
      </div>
