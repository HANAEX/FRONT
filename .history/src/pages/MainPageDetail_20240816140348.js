...
const MainPageDetail = () => {
  ...
  // 스크롤 이벤트를 감지하여 left-section 고정 및 버튼 배경색 변경
  useEffect(() => {
    const handleScroll = () => {
      const recommandSection = document.querySelector(`div[name="recommand-section"]`);
      const section1 = document.querySelector(`div[name="1-1"]`);
      const section2 = document.querySelector(`div[name="1-2"]`);
      const section3 = document.querySelector(`div[name="1-3"]`);
      const scrollPosition = window.scrollY + 50; // Adjust for slight offset

      if (recommandSection.getBoundingClientRect().top <= 0) {
        setIsFixed(true);
      } else {
        setIsFixed(false);
      }
      ...
      if (section3.getBoundingClientRect().top <= scrollPosition) {
        setActiveSection("1-3");
      } else if (section2.getBoundingClientRect().top <= scrollPosition) {
        setActiveSection("1-2");
      } else if (section1.getBoundingClientRect().top <= scrollPosition) {
        setActiveSection("1-1");
      } else {
        setActiveSection("");
      }
    };
    ...
    <div className="w-[960px] bg-slate-500 flex flex-col py-1 px-10">
      ...
      > 
        ...
        >
          <div className='flex items-center mb-1'>
            <Text className='text-2xl'>USD/KRW</Text>
            <Image boxSize={'42px'} src="/image/america_flag.png" alt="US Flag" />
          </div>

          <MainRecommandButton 
            text={"투자포인트"} 
            scrollToSection={() => scrollToSection("1-1")} 
            isActive={activeSection === "1-1"}
          />
          <MainRecommandButton 
            text={"추천주식"} 
            scrollToSection={() => scrollToSection("1-2")} 
            isActive={activeSection === "1-2"}
          />
          <MainRecommandButton 
            text={"추천상품"} 
            scrollToSection={() => scrollToSection("1-3")} 
            isActive={activeSection === "1-3"}
          />
        </div>
        <div name="right-section" className="bg-slate-50 p-5 flex-1">
          <div name="1-1" className="bg-slate-400 h-80 mt-6"></div>
