  ...
  return (
    <div className="w-[1140px] bg-slate-500 flex flex-col py-1 px-10 mt-10">
      <div className="bg-slate-200 mb-4">
        ...
        <Text size={'sm'} className='text-slate-700'>2024년 8월 12일 07:58</Text>
      </div>
      <div className="bg-slate-200 mb-4">
        <p className='font-semibold'>ChatGPT</p>
        <span>이 기사를 <span className='font-semibold text-red-500'>호재</span>로 분석했어요</span>
      </div>
      <Accordion allowToggle className="bg-slate-50 mb-4">
        <AccordionItem>
          <h2>
            <AccordionButton>
          ...
          <AccordionPanel pb={4}>
            ...
            1400원대 진입을 눈앞에 두고 있다. 주요국 통화 약세가 이어지며 당분간
            원/달러 환율은 오를 것이라는 관측이 나온다. 전날(24일) 서울
            외환시장에서 원/달러 환율은 전 거래일보다 0.7원 오른 1389.0원에
            거래가 마감됐다. 원/달러환율은 전 거래일보다 1.7원 오른 1390.0원으로
            개장한 뒤 오전 한때 1391.9원까지 상승했으나 점차 상승 폭을 줄였다.
            이날 원·달러 환율이 상승한 배경에는 스위스 중앙은행의 2회 연속 금리
            인하, 영국 영란은행(BOE)의 완화적 금리 동결 등 미국과의 통화정책
            차별화가 있다.
          </AccordionPanel>
        </AccordionItem>
      </Accordion>
      <div name="content-box" className="bg-slate-100 p-3">
        <Image src="/image/new_img.png" className="w-[500px] h-[250px]"></Image>
        {fulltext === true ? (
          <Text name="full-text">
            {full_text}
          </Text>
        ) : (
          <Text name="summary-text">
            {truncateText(full_text)}
          </Text>
        )}
