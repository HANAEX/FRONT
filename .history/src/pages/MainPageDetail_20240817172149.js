...
const MainPageDetail = () => {
  ...
  const [wonValue, setWonValue] = useState(currentInvestPrice);
  const [exchangeValue, setExchangeValue] = useState(currentInvestPrice); // ExChangeInput 값을 저장할 상태
  ...
  useEffect(() => {
    const getUseHistory = async () => {
      try {
        const result = await axios.get("https://a8068697-21bf-44f7-bf02-fccbc7115c44.mock.pstmn.io//hana/currentprice");
        setConsumData(result.data.data);
        setCurrentInvestPrice(result.data.data[0].invest);
        setCurrentSellPrice(result.data.data[0].sell);
        console.log(currentInvestPrice);
      } catch (error) {
        console.log(error);
      } finally {
